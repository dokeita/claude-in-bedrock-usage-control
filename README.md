...
1. **利用**: Developer が Claude (CLI / VS Code) 経由で Amazon Bedrock を呼び出し
2. **ログ収集**: Bedrock Model Invocation Logging が全リクエストを S3 に記録（`identity.arn` + トークン数）
3. **集計 (15分毎)**: Aggregator Lambda が S3 の invocation log からユーザーを特定し、トークン数を集計（処理済みファイルを追跡して重複カウント防止）
4. **閾値チェック**: DynamoDB の累計値が上限の 80% で警告通知、100% で Bedrock アクセスをブロック（IAM インラインポリシー Deny 付与）。通知は月に1回のみ送信
5. **月初リセット**: 毎月1日に累計値・通知フラグをゼロに戻し、Deny ポリシーを削除
